...
setwd("~/iowa_state/lab/RNAseq/DESeq2 HvR3/")
...
#countData=read.csv("~/iowa_state/lab/GRN_reconstruction/HORVUR2 version analysis/90_samples_barley_r2_bgh_genes_raw.csv", header=TRUE, row.names=1)[,-91]
countData=read.table("~/iowa_state/lab/RNAseq/assemblies HVR3/HvR3_Bgh_merged_salmon_gene_counts.tsv", header=TRUE, row.names = 1)[,-1]
#countData=read.table("~/iowa_state/lab/RNAseq/assemblies HVR3/HvR3_Bgh_merged_salmon_gene_counts.tsv", header=TRUE)[,-1]
...
#counts <- counts[grep("HORVU", rownames(counts), ignore.case=TRUE), ]
#counts <- counts[-grep("_LC", rownames(counts), ignore.case=TRUE), ]
#rownames(counts)[grep("HORVU", rownames(counts), ignore.case=TRUE)]<- do.call(rbind, strsplit(rownames(counts)[grep("HORVU", rownames(counts), ignore.case=TRUE)], "_"))[,1]
#rownames(counts)<- substr(rownames(counts),1,nchar(rownames(counts))-2)
#rownames(counts)<- do.call(rbind, strsplit(rownames(counts), "-"))[,1]
